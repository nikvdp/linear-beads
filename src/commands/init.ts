...
export const initCommand = new Command("init")
  ...
  .action(async (options) => {
    try {
      ...
      // New initialization stays on label scoping. Explicit existing project
      // configs are still honored below via getRepoScope().
      if (!hasRepoConfig()) {
        ...
        output("✓ Repo binding policy: v1 (label default)");
      }
