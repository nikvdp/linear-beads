...
import { closeIssue, updateIssue } from "../src/utils/linear";
...
async function runEval(
  ...
  const existingDescription =
    mode === "update_heals_backticked_literal_ref"
      ? "Backticked literal `LIN-4274` reference"
      : mode === "update_skips_backticked_model_ids"
        ...
        : mode === "update_skips_when_already_rich"
          ...
          : mode === "update_skips_backticked_generic_link"
            ? "Keep `[LIN-4274](https://linear.app/issue/LIN-4274)` literal"
  ...
  const fakeClient = {
    async request(query: string, variables: Record<string, unknown> = {}) {
      ...
      if (query.includes("mutation UpdateIssue")) {
        ...
        const state = input.stateId === "state-closed" ? closedState : openState;
        const responseDescription =
          (input.description as string | undefined) || existingDescription;
        ...
        return {
          issueUpdate: {
            ...
            issue: {
              id: "uuid-1",
              identifier: issueId,
              title: (input.title as string | undefined) || "Sample title",
  ...
  const originalTeamKey = process.env.LB_TEAM_KEY;
  const originalApiKey = process.env.LINEAR_API_KEY;
  process.chdir(cwd);
  try {
    process.env.LB_TEAM_KEY = "LIN";
    ...
    process.env.LB_TEAM_KEY = originalTeamKey;
    process.env.LINEAR_API_KEY = originalApiKey;
    process.chdir(originalCwd);
  }

  return {
    stdout: JSON.stringify({ capturedInputs, healReadCalls }),
    stderr: "",
    exitCode: 0,
  };
}
